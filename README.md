...
또는 [카톡 오픈 채팅방](https://open.kakao.com/o/gNplQr7f) 에 문의하세요.

주요 소식은 [페이스북 페이지](https://facebook.com/gimi9.page)에 공개합니다.

## 2025년 10월

* 공공데이터포털(data.go.kr) 복구 지원
*	AI 허브 수집 오류 수정
...
* 데이터셋에 포함된 리소스를 최대 1000개까지 표시하도록 제한
* SEO 개선
* 과도한 요청을 보내는 크롤러 속도 제한
* 파일명에 slash를 포함한 리소스의 미리보기 생성 오류 수정
...
* 농림축산식품 공공데이터 수집 수정, Open API 수집 추가
* 도로공사 고속도로 공공데이터 포털 수집 수정, Open API, LOD, 원문 추가
* 금융데이터거래소(findatamall) 수집 수정 (리뉴얼 반영)
* 사라진 데이터셋 조사, 삭제
* 해외 포털 업로드, 국가 필터 및 언어 필터 추가
* 스위스 데이터 포털 수집 수정
* 미리보기 없는 리소스의 "미리보기 생성 요청" 기능 추가
* [Geocoder](https://github.com/gisman/geocoder-kr) 오픈소스로 공개

1. EU 데이터 포털 (1,058,863데이터셋)
1. 미국 데이터 포털 (302,201데이터셋)
1. 일본 공공데이터 통합 검색 (175,733데이터셋)
1. 독일 데이터 포털 (123,324데이터셋)
1. 스페인 데이터 포털 (83,698데이터셋)
1. 이탈리아 데이터 포털 (67,884데이터셋)
1. 프랑스 공공 데이터 (60,470데이터셋)
1. 영국 데이터 포털 (54,271데이터셋)
1. 캐나다 데이터 포털 (42,008데이터셋)
1. 대만 타이베이시 (19,272데이터셋)
1. 스위스 데이터 포털 (12,805데이터셋)
1. 호주 데이터 포털 (9,240데이터셋)
1. 미국 뉴욕주 데이터 포털 (5,469데이터셋)
1. 미국 캘리포니아 주 데이터 포털 (4,148데이터셋)
1. 일본 데이터 포털 (2,340데이터셋)
1. 영국 런던시 데이터 포털 (1,303데이터셋)
1. 오스트리아 데이터 포털 (1,198데이터셋)
1. 홍콩 (847데이터셋)
1. 프랑스 모빌리티 (608데이터셋)
1. 프랑스 파리시 (427데이터셋)
...
* 서울시 공공데이터 포털 수집 오류 수정

## 2024년 10월

* 로그인 사용자에게 광고를 노출하지 않음.
* 부산 빅데이터 수집 (부산 빅데이터 혁신센터로 변경)
* 부산 공공데이터 포털 수집 수정 (리뉴얼 반영)

## 2024년 9월

* vworld 개선: notes 줄바꿈 개선, shp 미리보기 오류 수정

## 2024년 8월

* 공공데이터포털의 표준데이터 미리보기를 제거하고 HTML 바로가기로 변경

## 2024년 7월

* 경남 빅데이터 허브 수집 수정
* 농림축산식품 공공데이터포털 리뉴얼 반영
* vworld 리뉴얼 반영. 전체 재수집
* 충남 공공데이터(alldam) 수집 개선, 재실행
* 서버 장비 업그레이드. 빠름 빠름~

## 2024년 6월

* 런던, 캐나다 공공데이터 포털 추가 (실험)
* 지오코딩 배경 지도에 오픈스트리트맵 추가
* 동영상 미리보기 30초로 제한
* 공공데이터 포털 업데이트 주기를 주1회에서 매일로 변경
* 부산 빅데이터 플랫폼 업데이트 제외 (오랫동안 업데이트 안 되고 있음)
* shp 파일 미리보기 추가

1. 공공데이터포털 (81,952데이터셋)
1. 미국 (49,720데이터셋)
1. 캐나다 (40,887데이터셋)
1. 유통 빅데이터 (19,532데이터셋)
1. 대구광역시 (15,685데이터셋)
1. 금융데이터거래소 (13,236데이터셋)
1. 서울특별시 (8,045데이터셋)
1. 경기도 (4,793데이터셋)
1. 인천광역시 (4,600데이터셋)
1. 충청남도 (4,537데이터셋)
1. 소방안전 빅데이터 (3,119데이터셋)
1. 경상남도 (2,971데이터셋)
1. 전라북도 (2,651데이터셋)
1. 부산광역시 (2,617데이터셋)
1. 헬스케어 빅데이터 (2,021데이터셋)
1. 지역경제 빅데이터 (1,684데이터셋)
1. 농식품 빅데이터 (1,423데이터셋)
1. 문화 빅데이터 (1,398데이터셋)
1. 제주도 (1,301데이터셋)
1. 런던시 (1,190데이터셋)
1. 해양수산 빅데이터 (1,171데이터셋)
1. 환경 빅데이터 (1,107데이터셋)
1. 교통데이터 오픈마켓 (1,008데이터셋)
1. 라이프로그 빅데이터 (817데이터셋)
1. 디지털 산업혁신 빅데이터 (790데이터셋)
1. 산림 빅데이터 (789데이터셋)
1. AI 허브 (772데이터셋)
1. 공간융합 빅데이터 플랫폼 (756데이터셋)
1. 광주 빅데이터 (751데이터셋)
1. 농림축산식품 공공데이터 (708데이터셋)
1. 브이월드 (699데이터셋)
1. 통신 빅데이터 (696데이터셋)
1. 중소기업 빅데이터 (664데이터셋)
1. 스마트치안 빅데이터 (456데이터셋)
1. 스마트팜 빅데이터 플랫폼 (423데이터셋)
1. 연안 빅데이터 플랫폼 (423데이터셋)
1. 금융 빅데이터 (400데이터셋)
1. 서울특별시 빅데이터 (321데이터셋)
1. 부산 빅데이터 (319데이터셋)
1. 데이터안심구역 (275데이터셋)
1. 감염병 빅데이터 플랫폼 (266데이터셋)
1. 고속도로 공공데이터 포털 (245데이터셋)
1. 쿠콘 API (196데이터셋)
1. 부동산 빅데이터 플랫폼 (163데이터셋)
1. 데이터루트 KB 카드 (95데이터셋)
1. 모두의 말뭉치 (69데이터셋)
1. 카톨릭병원 빅데이터 (66데이터셋)
1. 통계데이터센터 (50데이터셋)
1. 공간빅데이터분석 플랫폼 (41데이터셋)
1. 한국관광 Data Lab (40데이터셋)
1. 인허가 (36데이터셋)
1. 데이터스 롯데카드 (25데이터셋)
1. 택지정보 (17데이터셋)
1. 피타그래프 (13데이터셋)
...
* 리버스 지오코딩 기능 추가: [리버스 지오코딩에 대해 아시나요?](https://gimi9.com/blog/reverse-geocoding/)
* 블로그 페이징 오류 수정
* 지도 미리보기의 최대 줌 아웃을 세계 전도로 확대
* HTML, LINK, API 아이콘 클릭하면 원본 페이지가 나오도록 개선


## 2024년 4월

* 파일 지오코딩 서비스 오픈(인코딩, 주소컬럼, 구분자 자동 감지, https://gimi9.com/blog/지오코딩에-대해-아시나요/)
* 주소 품질표 단순화: 실패한 주소만 표시
* 미리보기 누락된 데이터셋 조사, 다시 생성
* 별점 제거. 아무도 별점을 주지 않는다 ㅠㅠ
* 브이월드(vworld) 수집 수정. 미리보기 제공
* 소방 빅데이터 수집 오류 수정
* 모두의 말뭉치(corpas) site renewal 적용, dataset url 개선
* AI Hub(aihub) site renewal 적용

## 2024년 3월

* CSV, 엑셀 보기 개선. gridjs 적용
* 주소품질 보기 gridjs 적용
* 파일 지오코딩 제공
* 주소 변경분 일일 업데이트 (지오코딩)
